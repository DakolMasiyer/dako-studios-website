/**
 * Enrich + requalify the next 2 not-yet-enriched Motion leads (continuing after
 * requalify-subset.ts and requalify-next5.ts). Aborts up front if Firecrawl credits
 * are exhausted rather than burning failed calls.
 *
 * Run:  pnpm tsx scripts/requalify-next2.ts
 */
import fs from 'fs'
...
async function main() {
  ...
  if (before <= 0) {
    console.error('Firecrawl balance is 0 — aborting before spending anything. Refresh/top-up has not landed on the API yet.')
    process.exit(1)
  }
  ...
  const next2 = (leads || []).filter((l) => l.company && !ALREADY_DONE.has(l.company)).slice(0, 2)
  console.log(`Selected: ${next2.map((l) => l.company).join(', ')}\n`)

  for (const lead of next2) {
