# Dako Studios — Content Calendar: Month 1

**Version:** 1.0
**Date:** 2026-06-16
**Status:** Active
**Period:** June 19 – July 18, 2026
**Feeds from:** `03_STRATEGY_TEMPLATE.md` + `04_CONTENT_PILLARS_TEMPLATE.md`

---

## Ground Rules

- **Active platforms Month 1:** Instagram · TikTok · Twitter/X · Blog
- **LinkedIn:** Held — phases in Month 2 when proof bank is stronger
- **YouTube:** Held — phases in Month 2 once cadence is established
- **Assets:** Built from scratch each week — no pre-existing footage or graphics
- **Phase:** Month 1 = *Prove the Model* — Education 35%, Proof 25%, BTS 20%, Culture 15%, Industry 5%
- **Volume target:** 10–12 posts Week 1 → steady 10–12/week across Month 1
- **Posting windows (WAT):**
  - Instagram: Tue–Fri, 7–9pm (peak ~8pm)
  - TikTok: Tue–Fri, 2–6pm
  - Twitter/X: Tue/Wed ~9pm OR Tue–Thu 8–11am
  - Blog: Any day — SEO-driven, not time-sensitive

---

## Asset Creation Protocol (Zero-Asset Start)

Every piece of content this month gets made from these raw inputs — no excuses for not having footage:

| Content type | How to create it from scratch |
|---|---|
| IG Carousel | Canva — use brand colours, clean typography, no stock photos |
| TikTok | Phone camera, talking head or screen record — Figma/Framer open on screen |
| Twitter/X | Text only — no asset needed |
| Blog | Written directly in dako.studio CMS or Notion → publish |
| Quote card | Canva single graphic — 1080×1080 |
| Before/After | Da'anong site screenshot (after) + placeholder "before" frame |

**Week 1 priority asset to create before Friday:**
Da'anong site screenshot (desktop + mobile) — this is your first proof visual and unlocks 3 posts.

---

## Phase Arc — Month 1

| Week | Dates | Theme | Pillar weight |
|---|---|---|---|
| Week 1 | Jun 19–25 | Studio introduction + first proof drop | Education heavy, Proof anchor |
| Week 2 | Jun 26–Jul 2 | Educate Femi — the website problem | Education + BTS |
| Week 3 | Jul 3–9 | Build credibility — film proof + Adaeze story | Proof + Culture |
| Week 4 | Jul 10–16 | Deepen trust — process + Academy | BTS + Education |
| Buffer | Jul 17–18 | Catch-up / repurpose best performers | — |

---

## WEEK 1 — Jun 20–26
**Theme:** Studio Introduction + First Proof Drop
**Goal:** Establish Dako Studios exists, what it does, and prove it with Da'anong's outcome

---

### Friday June 19 — LAUNCH DAY (3 posts)

---

**POST 1 — Twitter/X · 9pm WAT**
**Pillar:** Culture
**Format:** Single tweet — studio launch announcement

```
Dako Studios is live.

Labs. Brand. Motion. Film. Academy.

One studio. Five arms. Built for businesses that can't afford to stay invisible.

dako.studio
```

*No hashtags. No emoji. Let it stand.*

---

**POST 2 — Instagram · 8pm WAT**
**Pillar:** Proof
**Format:** Single graphic — Da'anong case study card
**Asset needed:** Screenshot of dwain-gyang-dp.netlify.app (desktop hero) + stat overlay

**Visual:** Split — left side site screenshot, right side clean text block.
**Text block:**
```
Da'anong Gyang
Director of Photography

Brief: A site that books him film projects.
Result: More bookings.

ARRI-certified. Amazon Prime. M-Net.
Showmax. AMVCA nominee.

The site just made sure the right
people could find and verify that — fast.

Labs. dako.studio
```

**Caption:**
```
Da'anong was already one of Nigeria's most credentialed cinematographers.
The site just made sure the right people could find that — fast.

More film project bookings followed.

This is what a portfolio site should do. Not just look good.
Actually book work.

Built by Dako Studios Labs.
→ dako.studio
```

---

**POST 3 — TikTok · 4pm WAT**
**Pillar:** BTS
**Format:** Talking head, 45–60s — founder introduction

**Script:**
```
Hook (0–3s, on screen text): "Why I built a studio with 5 arms instead of just freelancing"

Body:
"My name is Dakol. I'm a UX designer based in Abuja.

For years I watched Nigerian businesses — real estate agents,
law firms, restaurants, film studios — lose leads and clients
because their web presence was either broken, outdated,
or didn't exist at all.

A freelancer could fix the website. But three months later
the same client needs a brand refresh. Then a product video.
Then a film campaign. And they're back to searching again.

So I built Dako Studios — five arms under one roof.
Labs for web. Brand for identity. Motion for video.
Film for campaigns. Academy to train the next generation.

One relationship. Everything they need to grow."

Close (last 3s): "This is Dako Studios. We just launched. Follow along."
```

---

### Saturday June 20 (1 post — light day)

---

**POST 4 — Twitter/X · 9am WAT**
**Pillar:** Education
**Format:** Thread, 5 tweets

```
Tweet 1:
If your only online presence is a Facebook page,
you're invisible to most people searching for you.

Here's why — and what to do about it. 🧵

Tweet 2:
Google doesn't rank Facebook pages for local search.

When someone types "property agent Abuja" or
"lawyer Lagos" into Google — your Facebook page
does not show up.

Your competitor's website does.

Tweet 3:
You don't own your Facebook audience.

Tomorrow Meta can change the algorithm,
restrict your reach, or suspend your account.

A website is yours. The data is yours.
No platform can take it.

Tweet 4:
A Facebook page signals "I'm not serious yet."

A professional website with your domain signals
credibility before a visitor reads a single word.

First impressions are structural, not cosmetic.

Tweet 5:
The fix isn't complicated.

A website that loads fast, has one clear CTA,
and is mobile-optimised will outperform a Facebook page
for lead generation every time.

That's what Dako Studios Labs builds.
→ dako.studio
```

---

### Sunday June 21 (rest — no post)

---

### Monday June 22 (1 post — warm-up week)

---

**POST 5 — Instagram · 8pm WAT**
**Pillar:** Education
**Format:** Carousel, 6 slides
**Asset:** Create in Canva — clean brand colours, bold typography

**Slide content:**
```
Slide 1 (Hook):
"5 things your website should do
before a visitor scrolls past
the first screen."

Slide 2:
01. A headline that says
exactly who you help
and what you do.

Not your company name.
Not a tagline. A sentence
that answers: "Am I in the right place?"

Slide 3:
02. One visible CTA
above the fold.

Call. Book. Get a quote.
One action. Not five.

Slide 4:
03. A trust signal
before they have to look for one.

Client logo. Testimonial. Chart result.
Something that says: others have trusted us.

Slide 5:
04. A layout that works
on a phone.

70%+ of your visitors
are on mobile.
If it breaks on mobile,
it's broken.

Slide 6:
05. A load time
under 3 seconds.

Every second after that,
you lose visitors.

Which of these is your site missing?
→ dako.studio
```

**Caption:**
```
Most SME websites lose the visitor before they even scroll.

Not because the design is bad.
Because the structure is wrong.

Save this. Run it against your current site.

→ Need a rebuild? dako.studio
```

---

### Tuesday June 23 (2 posts — peak day)

---

**POST 6 — TikTok · 3pm WAT**
**Pillar:** Education
**Format:** Screen record + voiceover, 45s
**Asset:** Open Figma or Framer on screen — show a blank canvas

**Script:**
```
Hook (0–3s): "This is where a website actually starts. Not here—"
[cut to: a Canva template or generic builder]
"—here."
[cut to: blank Figma canvas]

Body:
"Before a single colour gets chosen,
before a font is picked,
we map how your customer moves through the site.

Where does their eye go first?
What do they need to see before they trust you?
Where does the CTA have to live?

This is called UX — User Experience design.
Most template tools skip this entirely.
That's why template sites don't convert.

They look fine. They just don't work."

Close:
"Design follows behaviour. Not the other way around.
That's how Dako Studios builds."
```

---

**POST 7 — Twitter/X · 9pm WAT**
**Pillar:** Proof
**Format:** Single tweet — film numbers card

```
Numbers from First Features (Amazon Prime Nigeria):

1M+ Meta reach
43% email open rate
300% engagement growth
All films → #1 Nigeria Top Movies chart

Film marketing is about reaching the audience
that converts to viewers.

Not impressions. Viewers.

— Dako Studios Film
```

---

### Wednesday June 24 (2 posts — peak day)

---

**POST 8 — Instagram · 8pm WAT**
**Pillar:** BTS
**Format:** Stories sequence, 4 frames
**Asset:** Screenshots from Da'anong project — brief notes, site wireframe sketch (hand-drawn is fine), live site, contact page

```
Frame 1:
"The brief."
[Photo: handwritten or typed brief notes]
Caption: "A cinematographer with Amazon Prime credits
needed a site that books him work."

Frame 2:
"The structure."
[Photo: rough wireframe sketch or Figma lo-fi]
Caption: "Before design: map how a production company
searches for a DP."

Frame 3:
"The build."
[Screenshot: dwain-gyang-dp.netlify.app desktop]
Caption: "Live. Clean. Credentialed."

Frame 4:
"The outcome."
[Text card]
Caption: "More film project bookings.
That's what a portfolio site should do."
```

*Post as a Reel or carousel if Stories reach is low on a new account.*

---

**POST 9 — TikTok · 2pm WAT**
**Pillar:** Proof
**Format:** Talking head, 45s — Da'anong case study

**Script:**
```
Hook (0–3s): "This is what a portfolio site should actually do."

Body:
"Da'anong Gyang is a Director of Photography.
ARRI-certified. Amazon Prime credits.
AMVCA Best Cinematography nominee.

He had the credits. He needed a site that
made sure the right people could find
and verify that — fast.

We built it. It went live.
More film project bookings followed.

That's the only metric that matters
for a portfolio site. Not how it looks.
Whether it books work."

Close:
"Dako Studios Labs.
We build sites that work.
→ dako.studio"
```

---

### Thursday June 25 (2 posts)

---

**POST 10 — Instagram · 8pm WAT**
**Pillar:** Education
**Format:** Single graphic — quote card
**Asset:** Canva, 1080×1080, clean typography

```
Visual text:
"A website that looks good
and a website that works
are two different builds."

— Dako Studios
```

**Caption:**
```
Most SME websites pass the visual test.
They fail the conversion test.

The difference is UX thinking at the brief stage —
before a single design decision is made.

That's where we start.
→ dako.studio
```

---

**POST 11 — Twitter/X · 9am WAT**
**Pillar:** Culture
**Format:** Single tweet

```
Nigerian SMEs deserve design
that works as hard as they do.

Not a template.
Not a vanishing freelancer.
Not an agency they can't afford.

That's what Dako Studios is for.
```

---

### Friday June 26 (1 post — Week 1 close)

---

**POST 12 — Blog · dako.studio/blog**
**Pillar:** Education
**Format:** Long-form article, 800–1000 words
**SEO target:** "web design for small businesses Nigeria" / "website for Nigerian business"

**Title:** "Why your Facebook page is not a website — and what it's costing your business"

**Structure:**
```
Intro (100 words):
The scenario — a business owner in Lagos or Abuja
who has a Facebook page with 2,000 followers and
zero website. Why that's not the same thing as having
an online presence.

Section 1 — Google doesn't see your Facebook page (200 words):
Local search behaviour. What happens when someone
searches "property agent Lekki" or "lawyer Abuja."
Facebook pages don't rank. Websites do.

Section 2 — You don't own your audience (150 words):
Platform risk. Algorithm changes. Account restrictions.
Your website data is yours.

Section 3 — Trust signals and credibility (150 words):
What a domain name signals vs. a Facebook URL.
First impressions before a word is read.

Section 4 — What a website should actually do (200 words):
Not just exist — convert. What conversion means for
an SME: an enquiry, a call, a booking.
The 5-point checklist from the IG carousel.

CTA (100 words):
If your site isn't doing these things, it's not a
business asset — it's a digital placeholder.
Discovery call link → dako.studio
```

---

## WEEK 1 SUMMARY

| Day | Platform | Post # | Pillar | Format |
|---|---|---|---|---|
| Fri Jun 20 | Twitter/X | 1 | Culture | Launch tweet |
| Fri Jun 20 | Instagram | 2 | Proof | Da'anong case study card |
| Fri Jun 20 | TikTok | 3 | BTS | Founder intro video |
| Sat Jun 21 | Twitter/X | 4 | Education | Facebook vs website thread |
| Mon Jun 23 | Instagram | 5 | Education | 5-point website checklist carousel |
| Tue Jun 24 | TikTok | 6 | Education | UX thinking screen record |
| Tue Jun 24 | Twitter/X | 7 | Proof | Film numbers tweet |
| Wed Jun 25 | Instagram | 8 | BTS | Da'anong stories sequence |
| Wed Jun 25 | TikTok | 9 | Proof | Da'anong case study talking head |
| Thu Jun 26 | Instagram | 10 | Education | Quote card |
| Thu Jun 26 | Twitter/X | 11 | Culture | SME deserve design tweet |
| Fri Jun 27 | Blog | 12 | Education | Facebook vs website article |

**Total: 12 posts · 4 platforms**
**Pillar split: Education 5 · Proof 3 · BTS 2 · Culture 2**

---

## WEEK 2 — Jun 27–Jul 3
**Theme:** Educate Femi — The Website Problem in Depth
**Goal:** Establish Dako Studios as the trusted educator before the vendor


### Saturday June 27 (1 post)

---

**POST 13 — Twitter/X · 9am WAT**
**Pillar:** Education
**Format:** Thread, 5 tweets

```
Tweet 1:
Most website "handoffs" are a link and a password.
That's not a handoff — that's an exit. 🧵

Tweet 2:
A real handoff means you own the domain.
Not your developer. You.
If you stop paying them, your business doesn't disappear.

Tweet 3:
It means you can edit a price, swap a photo, update your hours — without calling anyone.
You shouldn't need an invoice to fix a typo.

Tweet 4:
It means there's documentation.
You should know how your digital storefront is built.

Tweet 5:
It means independence within a professionally designed structure.
That's the model we build on. Framer + client-editable CMS.
→ dako.studio
```

---

### Monday June 29 (2 posts)

---

**POST 14 — Instagram · 8pm WAT**
**Pillar:** Education
**Format:** Carousel, 5 slides
**Asset:** Create in Canva — clean brand colours

```
Slide 1:
Facebook page vs website — 4 structural differences

Slide 2:
01. Ownership
Facebook can change its algorithm tomorrow and kill your reach.
Your website is your own digital real estate.

Slide 3:
02. Searchability
Google doesn't rank Facebook pages for local searches.
If someone searches "lawyer in Abuja", your website ranks, your page doesn't.

Slide 4:
03. Trust
A custom domain (.com or .ng) signals you are established before a word is read.

Slide 5:
04. Analytics
A website tells you exactly where your leads drop off.
Facebook gives you vanity metrics.
→ dako.studio
```

---

**POST 15 — TikTok · 3pm WAT**
**Pillar:** BTS
**Format:** Screen record, 45s

```
Hook: "This is what a client-editable site actually looks like."
Body: (Screen record showing a Framer CMS interface)
"We're setting this up so the client can change their service prices without ever needing to call us again.
Most agencies build dependency into their relationship. We build independence."
Close: "Dako Studios. We build sites you own."
```

---

### Tuesday June 30 (2 posts)

---

**POST 16 — Twitter/X · 9pm WAT**
**Pillar:** Proof
**Format:** Single tweet

```
Da'anong Gyang is a DP with Amazon Prime credits.
We built him a portfolio site that makes sure the right people can verify that fast.
Result: More bookings. That's what a website is for.
```

---

**POST 17 — Instagram · 8pm WAT**
**Pillar:** BTS
**Format:** Reel

```
Hook: "Running a 5-arm studio as a solo founder."
Body: (Quick cuts: Figma wireframe -> Canva brand deck -> Academy capstone review)
"Today: 2 discovery calls for Labs, 1 brand deck revision, and reviewing Academy Day 20 capstones. No team yet. Just a system that holds."
Close: "Building in public. Follow along."
```

---

### Wednesday July 1 (2 posts)

---

**POST 18 — TikTok · 2pm WAT**
**Pillar:** Education
**Format:** Talking head, 45s

```
Hook: "Why your Wix site isn't converting — and it's not your fault."
Body: "Your Wix site looks fine. But it's missing UX thinking.
A template gives you colours. It doesn't tell a visitor where to look first, or where the CTA should live.
A site that looks good and a site that works are two different builds."
Close: "Fix the structure. → dako.studio"
```

---

**POST 19 — Twitter/X · 9am WAT**
**Pillar:** Culture
**Format:** Single tweet

```
Professional creative work was never only for big budgets.
The gap isn't skill. It's access to a studio that works at your scale.
```

---

### Thursday July 2 (2 posts)

---

**POST 20 — Instagram · 8pm WAT**
**Pillar:** Education
**Format:** Carousel, 4 slides
**Asset:** Canva, wireframe sketches

```
Slide 1:
What UX thinking looks like before design begins.

Slide 2:
Most agencies go straight to fonts and colours.
We start here: (Show low-fidelity wireframe)

Slide 3:
We map where the visitor's eye goes, what they need to see to trust you, and where the CTA lives.

Slide 4:
Design without this step is decoration, not strategy.
→ dako.studio
```

---

**POST 21 — TikTok · 4pm WAT**
**Pillar:** Proof
**Format:** Talking head, 45s

```
Hook: "We got two films to #1 and #2 on Prime Video Nigeria in 5 days."
Body: "Here is the campaign structure.
One unified theme across both films.
Coordinated PR, SEO, and paid strategies pointed at the same audience signal.
Film marketing isn't about impressions. It's about reaching the audience that converts."
Close: "Dako Studios Film. dako.studio"
```

---

### Friday July 3 (1 post)

---

**POST 22 — Blog · dako.studio/blog**
**Pillar:** Education
**Format:** Article, 800 words

```
Title: What a client-editable website actually means — and why it matters

Most web agencies build dependency into their projects. You need them to make a simple text change.
At Dako Studios, we hand over a Framer CMS that lets you:
- Update pricing
- Swap portfolio images
- Change business hours
All without writing a line of code or paying a retainer.
Independence is built into the deliverable.
```

---


## WEEK 3 — Jul 4–10
**Theme:** Build Credibility — Film Proof + Adaeze Story
**Goal:** Drop the big proof moments + introduce Academy via Adaeze


### Saturday July 4 (1 post)

---

**POST 23 — Twitter/X · 9am WAT**
**Pillar:** Proof
**Format:** Thread, 6 tweets

```
Tweet 1:
Two films. One launch window. Five days to #1 and #2 on Prime Video Nigeria. 🧵

Tweet 2:
The challenge — two separate PR agencies, two films, one shared release window.

Tweet 3:
The approach — one unified campaign theme across both, instead of two competing pushes.

Tweet 4:
Coordinated content, SEO, email, and paid — all pointed at the same audience signal.

Tweet 5:
The result — Kill Boro and A Father's Love hit #1 and #2 within 5 days of release.

Tweet 6:
This is what a coordinated film campaign looks like when reach and conversion are treated as the same problem.
— Dako Studios Film
```

---

### Monday July 6 (2 posts)

---

**POST 24 — Instagram · 8pm WAT**
**Pillar:** Proof
**Format:** Carousel, 4 slides
**Asset:** Canva, stat breakdown

```
Slide 1:
Numbers from First Features (Amazon Prime Nigeria)

Slide 2:
1M+ Meta reach
43% email open rate

Slide 3:
300% engagement growth
All films -> #1 Nigeria Top Movies chart

Slide 4:
Film marketing is about reaching the audience that converts to viewers. Not just impressions.
— Dako Studios Film
```

---

**POST 25 — TikTok · 3pm WAT**
**Pillar:** BTS
**Format:** Screen record, 45s

```
Hook: "Watch us map out a website before we ever open a design file."
Body: (Show Figma wireframe session live)
"We're blocking out the user journey.
Hero section -> Trust signals -> Services -> Clear CTA.
If this structure fails, the design fails.
We engineer the structure first."
Close: "That's how Dako Studios builds."
```

---

### Tuesday July 7 (2 posts)

---

**POST 26 — Twitter/X · 9pm WAT**
**Pillar:** Education
**Format:** Thread, 5 tweets

```
Tweet 1:
Before you put your website URL in your email signature, run this check. 🧵

Tweet 2:
1. Does it load in under 3 seconds?
If it doesn't, you've already lost half your visitors.

Tweet 3:
2. Does it work flawlessly on mobile?
70% of your traffic is on a phone.

Tweet 4:
3. Is there ONE clear CTA above the fold?
Don't make them scroll to know how to hire you.

Tweet 5:
4. Do you own the domain?
If you're on a free .wixsite link, you're signaling you aren't established yet.
Need a fix? → dako.studio
```

---

**POST 27 — Instagram · 8pm WAT**
**Pillar:** Proof
**Format:** Carousel, 4 slides
**Asset:** Canva, Double Bill case study

```
Slide 1:
Two films. One launch window.

Slide 2:
The Challenge: How do you market two major Prime Video releases simultaneously without them cannibalizing each other?

Slide 3:
The Approach: A unified digital campaign arc. Coordinated PR and paid strategies.

Slide 4:
The Result: #1 and #2 on Prime Video Nigeria within 5 days.
Reach the audience that converts.
— Dako Studios Film
```

---

### Wednesday July 8 (2 posts)

---

**POST 28 — TikTok · 4pm WAT**
**Pillar:** Culture
**Format:** Talking head, 45s

```
Hook: "What it means to build a creative studio in Abuja in 2026."
Body: "The talent is here. The clients are here. The infrastructure gap is closing fast.
We're building Dako Studios because Nigerian SMEs deserve world-class design that scales with them, without paying enterprise agency fees."
Close: "We build for this market because we're part of it."
```

---

**POST 29 — Twitter/X · 9am WAT**
**Pillar:** Proof
**Format:** Single tweet

```
Academy highlight: "I'd never touched Figma before. By Day 20, I built a full landing page from scratch." — Adaeze, Dako Studios Academy.
Real skills. Real projects.
```

---

### Thursday July 9 (2 posts)

---

**POST 30 — Instagram · 8pm WAT**
**Pillar:** Culture
**Format:** Single graphic
**Asset:** Canva quote card

```
"Professional creative work was never only for big budgets.
The gap isn't skill or ambition. It's access to a studio that works at your scale."
— Dako Studios
```

---

**POST 31 — TikTok · 2pm WAT**
**Pillar:** BTS
**Format:** Reel / Screen record, 60s

```
Hook: "What a student built in 20 days."
Body: (Walkthrough of Adaeze's Day 20 capstone project)
"This is Adaeze's final project. She started with zero design experience.
By Day 20, she submitted a fully responsive landing page prototype."
Close: "The bootcamp is live at learn.dako.studio."
```

---

### Friday July 10 (1 post)

---

**POST 32 — Blog · dako.studio/blog**
**Pillar:** Proof
**Format:** Case study, 1000 words

```
Title: How we ran the digital campaign for two simultaneous Prime Video releases

A deep dive into the Double Bill campaign for Natives Filmworks.
We cover the challenge of marketing Kill Boro and A Father's Love at the same time.
We break down the unified theme, the email strategy (43% open rate), and the paid approach that drove both films to the top 2 spots on the charts in under a week.
```

---


## WEEK 4 — Jul 11–17
**Theme:** Deepen Trust — Process + Academy
**Goal:** Show the studio's thinking layer; warm the Academy funnel


### Saturday July 11 (1 post)

---

**POST 33 — Twitter/X · 9am WAT**
**Pillar:** Education
**Format:** Thread, 5 tweets

```
Tweet 1:
The real reason Nigerian SME websites don't generate leads. 🧵

Tweet 2:
It's not the budget. It's the build process.
Most are built without a UX brief.

Tweet 3:
They have no conversion architecture.
A pretty homepage doesn't tell the visitor what to do next.

Tweet 4:
And there's no post-launch ownership.
If you can't edit it, it rots.

Tweet 5:
Start with the user journey. Build it on a platform you can control.
That's the Dako Studios model.
```

---

### Monday July 13 (2 posts)

---

**POST 34 — Instagram · 8pm WAT**
**Pillar:** BTS
**Format:** Carousel, 4 slides
**Asset:** Canva, project lifecycle

```
Slide 1:
A Labs build — from brief to handoff.

Slide 2:
Step 1: The Brief & UX Map
We don't touch colors until we know exactly who we are talking to.

Slide 3:
Step 2: The Framer Build
Engineering the site to be blazing fast and responsive.

Slide 4:
Step 3: The Handoff
We hand over the keys. You get an editable CMS and full domain ownership.
→ dako.studio
```

---

**POST 35 — TikTok · 3pm WAT**
**Pillar:** Education
**Format:** Talking head, 45s

```
Hook: "What actually happens in a discovery call with a web designer."
Body: "If they immediately ask you what colors you like, hang up.
A real discovery call is about your business.
Who is your best customer? What is the biggest objection they have?
We figure out the business problem before we design the digital solution."
Close: "Book a real discovery call. → dako.studio"
```

---

### Tuesday June 305 (2 posts)

---

**POST 36 — Twitter/X · 9pm WAT**
**Pillar:** Culture
**Format:** Single tweet

```
One relationship. Multiple capabilities.
Web, Brand, Motion, Film, Academy.
That's the Dako Studios model.
```

---

**POST 37 — Instagram · 8pm WAT**
**Pillar:** Education
**Format:** Carousel, 4 slides
**Asset:** Canva

```
Slide 1:
What Dako Studios Labs actually delivers.

Slide 2:
We deliver a website that loads under 3 seconds.

Slide 3:
We deliver an architecture engineered for your specific customer.

Slide 4:
We deliver full ownership. No retainers. No hostage domains.
→ dako.studio
```

---

### Wednesday July 15 (2 posts)

---

**POST 38 — TikTok · 4pm WAT**
**Pillar:** BTS
**Format:** Screen record, 45s

```
Hook: "Live look at a Framer build in progress."
Body: (Screen recording of Framer interface)
"We are adding subtle scroll animations to keep the user engaged without sacrificing performance.
This is where the magic happens—balancing aesthetics with speed."
Close: "Follow for more behind the scenes."
```

---

**POST 39 — Twitter/X · 9am WAT**
**Pillar:** Proof
**Format:** Single tweet

```
First Dako Studios Labs client site just went live.
Built in 21 days. Fully editable by the client.
Zero developer retainer needed.
```

---

### Thursday July 16 (2 posts)

---

**POST 40 — Instagram · 8pm WAT**
**Pillar:** Culture
**Format:** Carousel, 4 slides
**Asset:** Canva, ecosystem map

```
Slide 1:
The creative ecosystem we are proud to be part of.

Slide 2:
Nigerian Film — Elevating Nollywood marketing with data and design.

Slide 3:
African Tech — Building platforms for FoodTech and SaaS innovators.

Slide 4:
We build for this market because we're part of it.
— Dako Studios
```

---

**POST 41 — TikTok · 2pm WAT**
**Pillar:** Education
**Format:** Talking head, 45s

```
Hook: "3 questions to ask before hiring a web designer."
Body: "1. Do I own the domain and hosting?
2. Can I edit text and images myself after launch?
3. What is your UX process before design starts?
If they can't answer these, walk away."
Close: "Hire a studio that gives you ownership. → dako.studio"
```

---

### Friday July 17 (1 post)

---

**POST 42 — Blog · dako.studio/blog**
**Pillar:** Education
**Format:** Article, 800 words

```
Title: What to expect from a 21-day website sprint

A plain-English guide to our Labs process.
Week 1: Strategy and UX Wireframes.
Week 2: Design and Framer Build.
Week 3: Revisions, CMS Setup, and Handoff.
No endless scope creep. Just an engineered solution delivered in 3 weeks.
```

---


## MONTH 1 TOTALS

| Platform | Posts |
|---|---|
| Instagram | 16 |
| TikTok | 15 |
| Twitter/X | 13 |
| Blog | 4 |
| **Total** | **48** |

| Pillar | Posts | % |
|---|---|---|
| Education | 18 | 37.5% |
| Proof | 12 | 25% |
| BTS | 10 | 21% |
| Culture | 7 | 14.5% |
| Industry | 1 | 2% |

*On target for Month 1 phase weighting: Education 35%, Proof 25%, BTS 20%, Culture 15%*

---

## Copy Bank — Reusable Captions & Hooks

### IG Caption Closers (rotate these)
```
→ dako.studio
Built by Dako Studios Labs.
That's what a [website/brand/portfolio] should do.
Save this.
Which of these does your site fail?
```

### TikTok Hook Bank (first 3 seconds)
```
"Most [websites/SME owners/agencies] get this wrong."
"This is what [X] actually looks like."
"Before a single pixel gets designed—"
"Two films. One launch window. Five days."
"Your Wix site looks fine. So why isn't anyone calling?"
"This is where a website actually starts."
```

### Twitter/X One-Liner Bank
```
Nigerian SMEs deserve design that works as hard as they do.
A website that looks good and a website that works are two different builds.
Design follows behaviour. Not the other way around.
Film marketing is about the audience that converts. Not impressions.
One relationship. Multiple capabilities. That's the studio model.
```

---

## Pending Slots — Fill As They Materialise

| Slot | Waiting for | Unlocks |
|---|---|---|
| Week 4 Wed TW — milestone tweet | First Dako-brand client closed | Proof tweet + IG post |
| TK-02 handoff video | First Framer client handoff | Framer CMS walkthrough TikTok |
| PR-03 before/after carousel | First Framer site live | IG carousel + Stories |
| Academy enrollment post | First real student enrolled | Proof + BTS crossover |
| LinkedIn launch | Month 2 — proof bank stronger | Full LinkedIn rollout |
| YouTube | Month 2 — cadence established | Process walkthrough series |

---

## Week 1 Pre-Launch Checklist (Before Friday June 19)

- [ ] Screenshot Da'anong site — desktop hero + mobile view
- [ ] Write and design Post 2 (Da'anong case study card) in Canva
- [ ] Record TikTok 3 (founder intro) — phone camera, clean background, 60s max
- [ ] Draft Tweet 1 (launch tweet) — ready to post at 9pm Friday
- [ ] Set up dako.studio Instagram and TikTok accounts if not already live
- [ ] Write Blog Post 1 (Facebook vs website) — ready to publish Friday
- [ ] Confirm Da'anong stack for case study accuracy

---

**Next file:** `06_COPY_BANK_TEMPLATE.md` — full caption library, hook variants, and DM/email sequences for cold outreach.
